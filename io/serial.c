...
void nkeyboard_callback(NEVENTITEM item) {
...
//				TRACEOUT(("recv -> %02x", keybrd.data));
			...
			pic_setirq(1);
			nevent_set(NEVENT_KEYBOARD, keybrd.xferclock, nkeyboard_callback, NEVENT_RELATIVE);
		}
	}
}
...
void keyboard_ctrl(REG8 data) {
	...
	if (keybrd.ctrls < KB_CTR) {
		...
		if (!nevent_iswork(NEVENT_KEYBOARD)) {
			nevent_set(NEVENT_KEYBOARD, keybrd.xferclock, nkeyboard_callback, NEVENT_ABSOLUTE);
		}
	}
}
