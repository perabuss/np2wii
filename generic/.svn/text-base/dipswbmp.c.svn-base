...
static UINT8 *getbmp(const char *dat, DIPBMP *dipbmp) {
	...
	ret = (BMPFILE *)bmpdata_solvedata((unsigned char*)dat);
	if (ret == NULL) {
		goto gb_err1;
	}
...
UINT8 *dipswbmp_get9861(const UINT8 *s, const UINT8 *j) {
	...
	ret = getbmp((const char *)bmp9861, &dipbmp);
	if (ret) {
		for (i=0; i<3; i++) {
...
UINT8 *dipswbmp_getsnd26(UINT8 cfg) {
	...
	ret = getbmp((const char *)bmp26, &dipbmp);
	if (ret) {
...
UINT8 *dipswbmp_getsnd86(UINT8 cfg) {
	...
	ret = getbmp((const char *)bmp86, &dipbmp);
	if (ret) {
		for (i=0; i<8; i++) {
			x = i * 8 + 17;
			y = (cfg & (1 << i))?16:9;
...
UINT8 *dipswbmp_getsndspb(UINT8 cfg, UINT8 vrc) {
	...
	ret = getbmp((const char *)bmpspb, &dipbmp);
	if (ret) {
...
UINT8 *dipswbmp_getmpu(UINT8 cfg) {
	...
	ret = getbmp((const char *)bmpmpu, &dipbmp);
	if (ret) {
		for (i=0; i<4; i++) {
			x = (i + 2) * 9 + 1;
